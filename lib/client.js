...
Client.prototype._attachApi = function () {
  ...
  return new Promise(function (resolve, reject) {
    const Swagger = require("swagger-client"); // Certifique-se de que a versão é ^3.0.0

    // Monta a URL
    const ariUrl = util.format(
    ...
    // Conecta via Swagger moderno
    Swagger({
      url: ariUrl,
      requestInterceptor: (req) => {
        const basic = Buffer.from(`${self._connection.user}:${self._connection.pass}`).toString(
          "base64"
        );
      ...
      .then((client) => {
        self._swagger = client;

        // Sucesso na carga da API
        swaggerLoaded();
      })
      .catch((err) => {
    ...
    function swaggerFailed(err) {
      if (err && err.status === 401) {
        console.warn("ARI Authentication failed (401). Continuing without crashing.");
        self.emit("APILoadError", err);
        reject(err);
        return;
      }
      // self.emit("APILoadError", err);
      reject(err); // Outros erros ainda são tratados normalmente
    }
        ...
        function callSwagger(/* args..., callback */) {
          ...
          return new Promise(function (innerResolve, innerReject) {
            ...
            function swaggerError(err) {
              ...
              // Evitar crash em caso de 401
              if (err && err.message && err.message.includes("401")) {
                console.warn("ARI Client: Unauthorized (401) – conexão negada. Ignorando erro.");
                self.emit("APIUnauthorizedError", err);
                // Apenas resolve com null ou algo customizado
                innerResolve(null);
                return;
              }
